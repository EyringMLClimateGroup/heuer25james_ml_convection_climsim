# Adapted from https://github.com/leap-stc/ClimSim/blob/main/climsim_utils/data_utils.py
...
class data_utils:
    def __init__(self,
        ...
        self.v5cnv_inputs = ['state_t',
                            ...
                            # 'pbuf_CH4',
                            # 'pbuf_N2O',
                            'state_pmid', # todo change all to use pmid
                            'state_ps',
                            # 'pbuf_SOLIN',
                            'pbuf_LHFLX',
                            'pbuf_SHFLX',
        ...
        self.v5cnv_cons_inputs = ['state_t',
                            'state_rh',
                            'state_q0002',
                            'state_q0003',
                            'liq_partition',
                            ...
                            # 'pbuf_CH4',
                            # 'pbuf_N2O',
                            'dP', # todo change all to use pmid
                            'state_ps',
                            # 'pbuf_SOLIN',
                            'pbuf_LHFLX',
                            'pbuf_SHFLX',
        ...
        self.v5cnvqcqi_outputs = ['ptend_t',
                           'ptend_q0001',
                           'ptend_q0002',
                           'ptend_q0003',
                           'ptend_u',
        ...
        self.v5cnv_cons_outputs = ['ptend_t',
                           'ptend_q0001',
                           'ptend_q0002',
                           'ptend_q0003',
                           'ptend_u',
    ...
    def set_to_v5cnv_vars(self):
        '''
        This function sets the inputs and outputs to the V5cnv subset.
        It also indicates the index of the surface pressure variable.
        '''
        self.input_vars = self.v5cnv_inputs
        self.target_vars = self.v5cnv_outputs
        self.ps_index = 900
        self.input_feature_len = 916
        self.target_feature_len = 302
        self.full_vars = False
        self.full_vars_v5 = True
        
    def set_to_v5cnvqcqi_vars(self):
        '''
        This function sets the inputs and outputs to the V5cnv subset.
        It also indicates the index of the surface pressure variable.
        '''
        self.input_vars = self.v5cnvqcqi_inputs
        self.target_vars = self.v5cnvqcqi_outputs
        self.ps_index = 1080
        self.input_feature_len = 1096
        self.target_feature_len = 362
        self.full_vars = True
        self.full_vars_v5 = False
        
    def set_to_v5cnv_cons_vars(self):
        '''
        This function sets the inputs and outputs to the V5cnv subset.
        It also indicates the index of the surface pressure variable.
        '''
        self.input_vars = self.v5cnv_cons_inputs
        self.target_vars = self.v5cnv_cons_outputs
        self.ps_index = 1080
        self.input_feature_len = 1096
        self.target_feature_len = 362
        self.full_vars = True
    ...
    def get_target(self, input_file):
        ...
        if self.input_abbrev == 'mlexpandcnv' or self.input_abbrev == 'mlexpandconserv':
            tmp_input_vars = tmp_input_vars + ['state_t_phy', 'state_q0001_phy', 'state_q0002_phy', 'state_q0003_phy', 'state_u_phy', 'state_v_phy']
        ...
        if self.input_abbrev == 'mlexpandcnv':
            # print('Entering mlexpand case')
            # each timestep is 20 minutes which corresponds to 1200 seconds
            ds_target['ptend_t'] = ds_input['state_t_phy']
            ds_target['ptend_q0001'] = ds_input['state_q0001_phy']
            ds_target['ptend_u'] = ds_input['state_u_phy']
            ds_target['ptend_v'] = ds_input['state_v_phy']
            # save qn and qc/qi (also have to change vars for this)
            # ds_target['ptend_q0002'] = ds_input['state_q0002_phy']
            # ds_target['ptend_q0003'] = ds_input['state_q0003_phy']
            # ds_target['ptend_qn'] = ds_input['state_q0002_phy'] + ds_input['state_q0003_phy']
            if self.full_vars:
                ds_target['ptend_q0002'] = ds_input['state_q0002_phy']
                ds_target['ptend_q0003'] = ds_input['state_q0003_phy']
            elif self.full_vars_v5:
                ds_target['ptend_qn'] = ds_input['state_q0002_phy'] + ds_input['state_q0003_phy']
        elif self.input_abbrev == 'mlexpandconserv':
            ds_target['ptend_t'] = ds_input['state_t_phy']
            ds_target['ptend_q0001'] = ds_input['state_q0001_phy']
            ds_target['ptend_u'] = ds_input['state_u_phy']
            ds_target['ptend_v'] = ds_input['state_v_phy']
            if self.full_vars:
                ...
                raise Exception('For mlexpandconserv mode "self.full_vars" should be true')
    ...
    def save_as_npy(self,
        ...
        if self.save_zarr:
            ...
            da.from_array(npy_input).to_zarr(zarr_path)
        ...
        if self.save_zarr:
            ...
            da.from_array(npy_target).to_zarr(zarr_path)
